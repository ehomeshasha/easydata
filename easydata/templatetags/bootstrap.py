...
@register.filter
def bootstrap(element, label = ""):
    markup_classes = {'label': '', 'value': '', 'single_value': ''}
    return render(element, markup_classes, label)


@register.filter
def bootstrap_inline(element, label = ""):
...
def render(element, markup_classes, label):
    ...
    if element_type == 'boundfield':
        ...
        context = Context({'field': element, 'classes': markup_classes, 'label_list': label_list, 'element_type':element_type})
    else:
        ...
        if has_management:
            ...
            context = Context({'formset': element, 'classes': markup_classes, 'label_list': label_list, 'element_type':element_type})
        else:
            for field in element.visible_fields():
                add_input_classes(field)
            template = get_template("bootstrapform/form.html")
            context = Context({'form': element, 'classes': markup_classes, 'label_list': label_list, 'element_type':element_type})
